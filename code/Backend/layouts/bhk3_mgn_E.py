def generate(plot_height:int,plot_width:int) -> dict:
    ...
    rooms = {
        "Living": (18, 22),              # ~460
        "Living_Balcony": (3, 4),        # ~80
        "Kitchen": (12, 14),             # ~250
        "Normal_Bedroom": (10, 14),      # ~230
        "Guest_Bedroom": (9, 11),       # ~240
...
#layout logic
    ...
    mt_w = math.sqrt(rooms["Master_Toilet"] )
    ...
    living_w = (rooms["Living"] + rooms["Living_Balcony"]) / living_h
    ...
    bal_w = living_w
    bal_h = rooms["Living_Balcony"] / bal_w
    layout["Living_Balcony"] = Room(Point(PLOT_WIDTH - bal_w, PLOT_HEIGHT), Point(PLOT_WIDTH, PLOT_HEIGHT), Point(PLOT_WIDTH, PLOT_HEIGHT - bal_h), Point(PLOT_WIDTH - bal_w, PLOT_HEIGHT - bal_h), rooms["Living_Balcony"])

    mid_tw = PLOT_WIDTH - living_w
    guest_w = mid_tw * 0.4
    guest_h = rooms["Guest_Bedroom"] / guest_w
    layout["Guest_Bedroom"] = Room(Point(PLOT_WIDTH - living_w - guest_w, PLOT_HEIGHT), Point(PLOT_WIDTH - living_w, PLOT_HEIGHT), Point(PLOT_WIDTH - living_w, PLOT_HEIGHT - guest_h), Point(PLOT_WIDTH - living_w - guest_w, PLOT_HEIGHT - guest_h), rooms["Guest_Bedroom"])
...
#furniture logic
    def draw_furniture(ax, room_name, room):
        ...
        if room_name == "Living":
            ...
            sofa_y_base = room.a4.y + (living_h-bal_h)*0.4
            v_h, v_w, h_l = (living_h-bal_h)*0.6-living_w*0.18, living_w*0.18, living_w*0.6
            ...
            # TV Unit
            ax.add_patch(Rectangle((room.a2.x - WALL_THICKNESS-living_w*0.1, sofa_y_base), living_w*0.1, living_h*0.3, color=wood_color, ec=detail_color, zorder=20))

        elif room_name == "Kitchen":
            ax.add_patch(Rectangle((room.a2.x - kit_w*0.15-WALL_THICKNESS, room.a3.y+WALL_THICKNESS ), (kit_w*0.15), (room.a2.y - (room.a3.y)-1.2), color='#e2e8f0', ec='#94a3b8', zorder=20))#kitchen edge
            stove_x, stove_y = room.a2.x - kit_w*0.11-WALL_THICKNESS, room.a4.y + (room.a1.y - room.a4.y) / 2 - 1.5
        ...
        elif room_name == "Normal_Bedroom":
            b1_w, b1_h = min(nb_w*0.6, 12.0), min(nb_h*0.4, 8.0)
            bx = room.a4.x + 2.0
            ...
            b2_w, b2_h = min(mb_w*0.6, 12.0), min(mb_h*0.4, 8.0)
            ...
            # Pillows
            ax.add_patch(Rectangle((bx + b2_w*0.09, by + b2_h*0.2), b2_w*0.15, b2_h*0.2, color='white', ec='gray', lw=0.5, zorder=22))
            ax.add_patch(Rectangle((bx + b2_w*0.09, by + b2_h*0.6), b2_w*0.15, b2_h*0.2, color='white', ec='gray', lw=0.5, zorder=22))
            # Duvet
        ...
        elif room_name == "Guest_Bedroom":
            b3_w, b3_h = min(guest_w*0.7, 12.0), min(guest_h*0.4, 8.0)
    ...
    # Doors & Openings
    D_RAD = 1.7
    draw_opening_v(ax, PLOT_WIDTH - living_w, ((living_h-guest_h)*0.5 + (PLOT_HEIGHT - living_h)) , min(living_h-guest_h,4.0))#living
    draw_opening_h(ax, (PLOT_WIDTH-living_w)-(kit_w-living_w)*0.5, kit_h, min(kit_w-living_w, 4.0)) #kitchen
    ...
    draw_door(ax, PLOT_WIDTH , kit_h+ living_h*0.2, D_RAD, 90, 180)   #entry door
    draw_door(ax, PLOT_WIDTH*0.95 , PLOT_HEIGHT-bal_h, D_RAD-1, 90, 180) #balcony

    #window placement
    draw_opening_v(ax, 0, PLOT_HEIGHT*0.9,min(4,nb_h*0.4)) #normal
    draw_opening_h(ax, (nb_w+guest_w*0.4), PLOT_HEIGHT, min(4,guest_w*0.4)) #guest
    draw_opening_v(ax, 0, (mb_h+bath_h+mt_h*0.6),toilet_h*0.3) #toilet
    draw_opening_v(ax, 0, (mb_h+bath_h*0.6),bath_h*0.3) #bathroom
    draw_opening_v(ax, 0, (mb_h-mt_h*0.6),mt_h*0.3) #master toilet
    draw_opening_h(ax, (mb_w*0.5), 0, min(5,mb_w*0.4)) #master
    draw_opening_h(ax, (mb_w+ kit_w*0.5), 0, min(5, kit_w*0.4)) #kitchen
