def generate(plot_height: int, plot_width: int) -> dict:
    ...
    model.Maximize(area_vars["Living"])
    ...
    layout["Living"] = Room(Point(PLOT_WIDTH - living_w, PLOT_HEIGHT), 
    ...
    layout["Pooja"] = Room(Point(PLOT_WIDTH - pooja_w, PLOT_HEIGHT), 
                           Point(PLOT_WIDTH, PLOT_HEIGHT), 
                           Point(PLOT_WIDTH, PLOT_HEIGHT - pooja_w), 
                           Point(PLOT_WIDTH - pooja_w, PLOT_HEIGHT - pooja_w), 
                           rooms["Pooja"])
    ...
    layout["Kitchen"] = Room(Point(PLOT_WIDTH - kb_width, PLOT_HEIGHT - living_h), 
                             Point(PLOT_WIDTH, PLOT_HEIGHT - living_h), 
                             Point(PLOT_WIDTH, PLOT_HEIGHT - living_h - kitchen_h), 
                             Point(PLOT_WIDTH - kb_width, PLOT_HEIGHT - living_h - kitchen_h), 
                             rooms["Kitchen"])

    bal_k_h = rooms["Balcony_K"] / kb_width
    layout["Balcony_K"] = Room(Point(PLOT_WIDTH - kb_width, bal_k_h), 
                               Point(PLOT_WIDTH, bal_k_h), 
                               Point(PLOT_WIDTH, 0), 
                               Point(PLOT_WIDTH - kb_width, 0), 
                               rooms["Balcony_K"])
    ...
    layout["Bedroom1"] = Room(Point(bb_width - bed1_w, bb_height), 
                              Point(bb_width, bb_height), 
                              Point(bb_width, 0), 
                              Point(bb_width - bed1_w, 0), 
                              rooms["Bedroom1"])
    layout["Balcony_B"] = Room(Point(0, bb_height), 
                               Point(rooms["Balcony_B"] / bb_height, bb_height), 
                               Point(rooms["Balcony_B"] / bb_height, 0), 
                               Point(0, 0), 
                               rooms["Balcony_B"])

    #bedroom2+toilet+bathroom
    bed2_w = PLOT_WIDTH - living_w
    bed2_h = rooms["Bedroom2"] / bed2_w
    layout["Bedroom2"] = Room(Point(0, PLOT_HEIGHT), 
                              Point(bed2_w, PLOT_HEIGHT), 
                              Point(bed2_w, PLOT_HEIGHT - bed2_h), 
                              Point(0, PLOT_HEIGHT - bed2_h), 
                              rooms["Bedroom2"])

    free_h = PLOT_HEIGHT - bed2_h - bb_height
    toilet_h= free_h/2
    toilet_w = rooms["Toilet"] / (free_h/2)
    layout["Toilet"] = Room(Point(0, PLOT_HEIGHT - bed2_h), 
                            Point(rooms["Toilet"] / (free_h/2), PLOT_HEIGHT - bed2_h), 
                            Point(rooms["Toilet"] / (free_h/2), bb_height + free_h/2), 
                            Point(0, bb_height + free_h/2), 
                            rooms["Toilet"])
    bath_h= free_h/2
    bath_w = rooms["Bathroom"] / (free_h/2)
    layout["Bathroom"] = Room(Point(0, bb_height + free_h/2), 
                              Point(bath_w, bb_height + free_h/2), 
                              Point(bath_w, bb_height), 
                              Point(0, bb_height), 
                              rooms["Bathroom"])
    ...
    def draw_furniture(ax, room_name, room):
        ...
        if room_name == "Living":
            ...
            sofa_y_base = room.a4.y + (living_h-WALL_THICKNESS)*0.4
            v_h, v_w, h_l = (living_h-WALL_THICKNESS)*0.6-living_w*0.18, living_w*0.18, (living_w-pooja_w)*0.8
            ...
            # TV Unit
            ax.add_patch(Rectangle((room.a2.x - living_w*0.6, PLOT_HEIGHT-living_h+WALL_THICKNESS), (living_w*0.35), living_h*0.1, color=wood_color, ec=detail_color, zorder=20))
    ...
    for name, room in layout.items():
        ...
        elif name == "Bedroom2": draw_door(ax, bed2_w-(bed2_w-bath_w)*0.5, room.a4.y, 1.5, 0, 90)
        elif name == "Toilet" or name == "Bathroom": draw_door(ax, room.a3.x, room.a4.y + h_curr * 0.4, 1.0, 90, 180)
        elif name == "Pooja": draw_door(ax, room.a4.x + w_curr * 0.5, room.a4.y, 1.2, 90, 180)
        elif name == "Living":draw_door(ax,PLOT_WIDTH,PLOT_HEIGHT-living_h*0.8,1.5,90,180)
    ...
    draw_opening_h(ax, PLOT_WIDTH-living_h*0.6, PLOT_HEIGHT, 4)#living
    draw_opening_v(ax, (PLOT_WIDTH-kb_width), (kb_height-(kb_height-bb_height)*0.5), min(kb_height-bb_height,3)) #kitchen-opening
    draw_opening_v(ax,(PLOT_WIDTH-living_w), (PLOT_HEIGHT-bed2_h-(living_h-bed2_h)*0.5),min((living_h-bed2_h)*0.5,3))#living-opening
