def generate(plot_height: int, plot_width: int) -> dict:
    ...
    def percent(var, low, high):
        model.Add(var * 100 >= low * PLOT_AREA)
        model.Add(var * 100 <= high * PLOT_AREA)

    percent(living,   36, 40)
    percent(bedroom,  25, 32)
    percent(kitchen,  18, 20) 
    percent(bathroom,  8, 10)  
    percent(toilet,    4, 6)
    ...
    if status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        raise Exception("No feasible layout found")
    ...
    layout["Living"] = living_room

    # Kitchen + Gallery 
    KG_area = rooms["Kitchen"] + rooms["Gallery"]
    KG_h = PLOT_HEIGHT - living_h
    KG_w = KG_area / KG_h

    kitchen_h = rooms["Kitchen"] / KG_w

    kitchen_room = Room(
        Point(PLOT_WIDTH-KG_w, PLOT_HEIGHT-living_h),
        Point(PLOT_WIDTH, PLOT_HEIGHT-living_h),
        Point(PLOT_WIDTH, living_room.a3.y-kitchen_h),
        Point(PLOT_WIDTH-KG_w, living_room.a3.y-kitchen_h),
        rooms["Kitchen"]
    )
    layout["Kitchen"] = kitchen_room

    gallery_room = Room(
        Point(PLOT_WIDTH-KG_w, kitchen_room.a3.y),
        Point(PLOT_WIDTH, kitchen_room.a3.y),
        Point(PLOT_WIDTH, 0),
        Point(PLOT_WIDTH-KG_w, 0),
        rooms["Gallery"]
    )
    layout["Gallery"] = gallery_room

    # Bedroom
    bed_w = living_room.a1.x
    bed_h = rooms["Bedroom"] / bed_w

    bedroom_room = Room(
        Point(0, PLOT_HEIGHT),
        Point(bed_w, PLOT_HEIGHT),
        Point(bed_w, PLOT_HEIGHT-bed_h),
        Point(0, PLOT_HEIGHT-bed_h),
        rooms["Bedroom"]
    )
    layout["Bedroom"] = bedroom_room
    ...
    def draw_label(ax, name, room):
        ax.text(
            room.a4.x + (room.a2.x-room.a1.x)/2,
            room.a4.y + (room.a1.y-room.a4.y)/2,
            name,
            ha="center", va="center",
    ...
    def draw_furniture(ax, name, room):
        ...
        if name == "Bedroom":
            ...
            ax.add_patch(Rectangle((room.a2.x - KG_w*0.15- WALL_THICKNESS, room.a3.y + WALL_THICKNESS), (KG_w*0.15), (kitchen_h-2*WALL_THICKNESS), color='#e2e8f0', ec='#94a3b8', zorder=20))
            stove_x, stove_y = room.a2.x - KG_w*0.15, room.a4.y + (room.a1.y - room.a4.y) / 2 - 1.5
            ax.add_patch(Rectangle((stove_x, stove_y), KG_w*0.1, kitchen_h*0.3, color='#334155', zorder=21))
            for i in range(3):
                ax.add_patch(Circle((stove_x + KG_w*0.05, stove_y + kitchen_h*0.04 + i*(kitchen_h*0.25/3)), kitchen_h*0.025, color='black', ec='white', lw=0.3, zorder=22))
           
            t_width, t_height = KG_w*0.3, kitchen_h*0.2 
            ...
            if PLOT_WIDTH > PLOT_HEIGHT:
                ax.add_patch(Rectangle((PLOT_WIDTH-KG_w*0.8, room.a3.y + WALL_THICKNESS),KG_w*0.8-WALL_THICKNESS, living_h*0.15, color=fabric_color, ec=detail_color, lw=1, zorder=20))#horizontal-sofa
                for i in range(int(KG_w*0.8/(living_h*0.15))-2):
                    ax.add_patch(Rectangle((PLOT_WIDTH-KG_w*0.8 + 0.5 + (i*(living_h*0.2)), room.a3.y + WALL_THICKNESS), living_h*0.15, living_h*0.07, fill=False, ec=detail_color, lw=0.5, zorder=21))#cushion
            else:
            ...
            ax.add_patch(Rectangle((tx, ty), living_w*0.2, living_h*0.2, facecolor=wood_color, edgecolor='#5c4033', lw=1.5, zorder=22))
            ax.add_patch(Rectangle((PLOT_WIDTH-living_w*0.8, (PLOT_HEIGHT-living_h*0.06-WALL_THICKNESS)), living_w*0.3, living_h*0.06, color=wood_color, ec=detail_color, zorder=20)) #tv
    ...
    draw_opening_v(ax, PLOT_WIDTH, (KG_h-kitchen_h*0.5), min(kitchen_h*0.4,4))#kitchen
    draw_opening_v(ax, PLOT_WIDTH, PLOT_HEIGHT-living_h*0.5, min(living_h*0.4,5)) #living
    ...
    if PLOT_WIDTH > PLOT_HEIGHT:
        draw_door(ax, bed_w, bath_h+(bed_h-living_h)*0.6, D_RAD-0.5, 180,270)
    else:
        draw_door(ax, bath_w+(bed_w-bath_w)*0.5, layout["Bedroom"].a3.y, D_RAD-0.5, 0,90)
    
    draw_door(ax, layout["Kitchen"].a4.x + KG_w*0.3 + kitchen_h*0.4+WALL_THICKNESS, layout["Kitchen"].a4.y, D_RAD-0.5, 90, 180)#kitchen-balcony
    draw_door(ax, layout["Bathroom"].a2.x,  toilet_h+(bath_h-toilet_h)*0.6, D_RAD-1, 180, 270) #bathroom
    draw_door(ax, bath_w+toilet_w*0.5, layout["Toilet"].a1.y, D_RAD-1, 270, 360)
    draw_door(ax, PLOT_WIDTH-living_w*0.3, PLOT_HEIGHT, D_RAD-0.5, 270, 360) #living
